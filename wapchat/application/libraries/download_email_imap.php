<?
/**************************************************************************************
* FileName: download_email_imap.php
...
include("/var/www/html/patanjali/include/class-phpmailer.php");
define ("DOWNLOADPATH", "/var/www/html/patanjali");
define ("DBG","1");

/************************************************************************************************/

/* open database for queue entry */
$link = mysql_connect("192.168.1.18","cron","1234") or die("Could not connect: " . mysql_error());

//$link = mysql_connect("192.168.1.18","root","All1@nc3@1986!") or die("Could not connect: " . mysql_error());

mysql_select_db("patanjali_master");
...
***************************************************************************************/
$sql_connect_result="SELECT v_ipaddress,v_username,v_pasowrd FROM patanjali_master.tbl_connection";
...
//	$hostname				=$row_result_connect['v_ipaddress'];
	...
	//echo "stopping ".$hostname."  ".$username." ".$password;
	...
	if($emails) 
	{
		...
		foreach($emails as $email_number) 
		{
			...
			***********************************************************************************/
			$sql_qry = "SELECT i_UID FROM patanjali_master.web_email_information where i_UID = '".$iUID."'";
			...
			{ 
				...
				if(strtolower($part->disposition) == "attachment") 
				{
					...
					if ( strstr($filename,"UTF"))
					{
						...
						$filePath = tempnam("/var/www/html/patanjali/imap","attach_");
					...
					else
					{
						...
						$filePath= "/var/www/html/patanjali/".$filename;
...
//exit();
			...
			$sql_insert_information="INSERT INTO patanjali_master.web_email_information 
							(v_toemail,v_fromemail,d_email_date,v_subject,Filter_bySubject,Filter_byFrom,
								V_rule,v_body,email_type,ICASEID,i_Update_status,original_subject,email_test, i_UID) 
							VALUES ('$to','$from','$date','$subject','$subject','$from',
									'$multifiles','$body','IN','','','$entry_log','email_script_page', $iUID)";  


			echo "<br><br>";

			$result_insert=mysql_query($sql_insert_information) ;
			if ( $result_insert == FALSE)
			{
				echo "Unable to insert:".$sql_insert_information."Error:".mysql_error();
				print_r($header);
				print("\n");
exit();
