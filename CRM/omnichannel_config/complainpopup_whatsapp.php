...
$query5 = mysqli_query($link,"select * from $db.whatsapp_in_queue where id='$Mid'");
$query5 = mysqli_fetch_array($query5);

$v_body=$query5['message'];
$ICASEID=$query5['ICASEID'];
$i_DeletedStatus=$query5['i_DeletedStatus'];
$send_from=$query5['send_from'];

$q=mysqli_fetch_row(mysqli_query($link,"select AccountNumber from $db.web_accounts where whatsapphandle = '$send_from' and whatsapphandle!=''"));
$customerid = $q[0];

$new_case_manual = base64_encode('new_case_manual');
if ($customerid != '') {
    $link_images = "onclick=\"window.open('../helpdesk_index.php?token=$new_case_manual&customerid=$customerid&whatsappid=$Mid&mr=4', '_blank');\"";
} else {
    $link_images = "onclick=\"window.open('../helpdesk_index.php?token=$new_case_manual&whatsappid=$Mid&mr=4', '_blank');\"";
}
// get dispostion table details 
$query_dis = mysqli_query($link,"select * from $db.multichannel_disposition where channel_id='$Mid' and channel_type='Whatsapp'");
