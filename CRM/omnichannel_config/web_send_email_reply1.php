...
* Updated By Farhan Akhtar For Content Messaging Add-On on 16-10-2024
**/
...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $V_Subject = (is_int($pos)) ? $V_Subject : "[TICKET # ".$_REQUEST['iid']."] > ".$V_Subject;
    ...
    <table align='center' width="90%" border="0" cellspacing="0">
        <tr class='mail-pagination'>
            <td valign=middle align='left' class='normaltextabhi'>
                <input type="submit" name="addbtn_email" id="compose" class="compose" value="Send" onclick='javascript:return validate();' style='float:left; width:auto; margin-bottom:4px;'>
                <?php
        ...
        <tr>
            <td>
                ...
                <table width="100%" border="1" cellpadding="4" cellspacing="1" align="center" class="tbl" style='border-collapse: collapse;' bordercolor='#e3e2e2'>
                    <tr>
                        <td valign="top"><font color="#000000" face=verdana size=2>To Email-Id<span class="orangeTxt">*</span></font></td>
                        <td>
                            <input type="text" name="V_EmailId" id="V_EmailId" value="<?=$reply_to?>" class="textbox" size="40" style="<?=$dis?> margin-top: 2px;" placeholder="To :">
                        </td>
                    </tr>

                    <tr>
                        <td><font color="#000000" face=verdana size=2>Subject<span class="orangeTxt">*</span></font></td>
                        <td><?php $reply_subject = $reply_subject; ?><input type="text" name="V_Subject" class="textbox" size="40" value="<?= $reply_subject ?>" ></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <!-- Farhan Akhtar (16-10-2024) -->
                        <td ><font face="verdana" size=2><?= ($action != "") ? date("d-m-Y H:i:s") : date("d-m-Y H:i:s", strtotime($d_email_date)) ?></font></td>         
                        <!-- Farhan Akhtar (16-10-2024) -->
                    </tr>

                    <tr>
                        <td><font color="#000000" face=verdana size=2>Template<span class="orangeTxt">*</span></td>
                        <td>
                            <?php $query = $link->query("SELECT `default_template` from $db.`email_templates` WHERE `template_name`='reply_mail' AND `status`=1 "); ?>
                            <select name="template" id="template">
                                <option value="">Select template</option>
                                <?php 
                                $count = 0;
                                while($row = $query->fetch_assoc()) :
                                    $template_name = $link->real_escape_string($row['default_template']);
                                    $count++;
                                ?>
                    ...
                    <tr>
                        <td colspan='2'>
                            <textarea name="V_Content" rows="8" cols="40" style="width:100%; height:300px" id="V_Content" ></textarea>
                        </td>
                    </tr>   
                    <tr>
