...
$log_prefix = "[Facebook Messenger Script]";
...
if (mysqli_num_rows($results) > 0){
    while ($company = $results->fetch_assoc()) {
        ...
        Message_Outgoing($childdb); //For outgoing Data 
    }
}else{
...
*/
function Message_Outgoing($childdb){
	global $link,$childdb,$log_prefix,$type,$base_path;
	/* facebook access token and url form database */
	$sql_cdr= "SELECT * from $childdb.tbl_messenger_connection where status=1 and debug=1 ";
	$query=mysqli_query($link,$sql_cdr);
	$config = mysqli_fetch_array($query);
    if (!$query){
        if (__DBGLOG__){
                DbgLog(_LOG_ERROR,__LINE__, __FILE__,"$log_prefix DB fetching connection error tbl_messenger_connection: $sql_cdr". mysqli_error($link),'omichannel');
        }
        $response['error'] = TRUE;
        $response['error_msg'] = "$log_prefix DB fetching connection error tbl_messenger_connection: $sql_cdr";
	...
	$sql_cdr = "SELECT * FROM $childdb.messenger_out_queue WHERE status= '0'";
	$query=mysqli_query($link,$sql_cdr);
	...
	if($count != 0 ){
	  while($data=mysqli_fetch_assoc($query)){
	  		...
	  		$attachmentUrlDB = $data['attachment']; // Attachment URL (if any)
	  		$id = $data['id'];
            $url = $global_url.$page_id.'/messages';

	  		if ($attachmentUrlDB) {

	  			$attachmentUrl = $base_path . $attachmentUrlDB; // URL to the image file
                echo $attachmentUrl;
                ...
                // $attachmentId = uploadAttachmentToFacebook($attachmentUrlDB, $attachmentType, $page_access_token,$page_id,$global_url);
			...
			if(!empty($comment_id)){
				...
				$strQrytest ="update $childdb.messenger_out_queue set message_unique_id='".$comment_id."',sent_date='".$sent_date."',status='1',attachment_id ='".$attachmentId."' where id=".$id; // store commnet_id and send date also send flag 1
                ...
                $strQrytest ="update $childdb.messenger_out_queue set sent_date='".$sent_date."',status_response='".$status_response."',status='2' where id=".$id;
                echo"<br/>";echo"<br/>";echo $strQrytest;echo"<br/>";echo"<br/>";
                mysqli_query($link, $strQrytest);
                if (__DBGLOG__){
...
function uploadAttachmentToFacebook($image_url, $attachmentType, $page_access_token,$page_id,$global_url) {
    ...
    global $log_prefix,$type,$messenger_path_out;
    ...
    $temp_image_path = $messenger_path_out.$image_url;
    print_r($temp_image_path);
