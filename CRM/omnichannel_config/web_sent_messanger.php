 <?php 
/***
    * Messager Outgoing Message
    * Author: Aarti Ojha
    * Date: 08-08-2024
    * This file handles Messenger Outgoing Message 
    * To integrate Messenger messaging in your PHP application using the Facebook Business API, you need to follow several steps, including setting up your Facebook Developer account, creating a Messenger Business Account, configuring your webhook, and handling incoming and outgoing messages
    * 
...
$query_dis = mysqli_query($link,"select * from $db.multichannel_disposition where channel_id='$messageid' and channel_type = 'Facebook Messenger'");
...
// mysqli_query($link,"update $db.messenger_in_queue set flag='1' , status='1' where id='$ID'; ");
mysqli_query($link,"update $db.messenger_in_queue set flag='1' where send_from='$send_from'");

// Get sentiment value from the database
$query_sentiment = mysqli_query($link, "SELECT sentiment FROM $db.messenger_in_queue  WHERE id = '$messageid'");
      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="form_area">         
            ...
            <?php
              // $sqldm="SELECT id, send_to, send_from, message, create_date, channel_type,attachment FROM $db.messenger_out_queue WHERE send_to ='".$send_to."' UNION SELECT id, send_to, send_from, message, create_date, channel_type,attachment FROM $db.messenger_in_queue WHERE send_from ='".$send_to."' ORDER BY create_date asc";
                        $last_message_id = 0; // initialize
                        $sqldm = "
                        SELECT id, send_to, send_from, message,  create_date,  channel_type, attachment 
                        FROM (
                            SELECT 
                                id, 
                                send_to, 
                    ...
                    if($numrec){
                      ...
                      $sqlss = "SELECT id from  messenger_in_queue WHERE send_from = '".$send_to."' ORDER BY create_date DESC LIMIT 1";
                  ...
                  <form name="dm_form_msg" id="dm_form_msg"   enctype="multipart/form-data" method="post"  >
                      ...
                      <div class="chat_box_wrap">
                        <div class="header_t">
                          ...
                          <?php
                           function getMessengerUsername($send_to) {
                              global $db,$link;
                              $sql = "SELECT fname FROM $db.web_accounts WHERE messengerhandle = '$send_to'";
                          ...
                          $username = getMessengerUsername($send_to);
                          ?>
                            ...
                            if($numrec){
                              while($rm=mysqli_fetch_assoc($qdm)){
                                if($rm['send_to']!=$send_to){
                                    ...
                                    $msg_float="left";
                                    $symbol=">>>";
                                    $imgsrc='<img src="'.$SiteURL.'public/images/facebook-messenger-icone.png" alt="" style="height: 30px;width:30px;" title="user name">';
                                    $class ='received';
                                }else{
                                    $symbol=">>>";
                                      $msg_class="right-img";
          ...
          <div class="response_area"><div id="response_msg"><?=$msg?></div></div>
            <div class="form_area">
              <form name="dm_form" id="dm_form" enctype="multipart/form-data" method="post">
                <?php
                $sql="select * from $db.web_template_Messenger where i_type=1 and i_status =1";
                ...
                <tbody>
                  ...
                  <tr>
                    <td>
                      ...
                      <input type="hidden" name="channel_type" id="channel_type" value="Facebook Messenger">
...
// Function to scroll the chat to the bottom [Aarti][17-08-2024]
  ...
  function sendMessage(e) {
      ...
      formData.append("action", "messenger_reply");
...
// Messenger chat UI refreshes all messages every 4 seconds  [Vastvikta][14-05-2025]
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.html.trim() !== '') {
        ...
        console.log("[MessengerDM_Live] No new messages.");
      }
    },
    error: function (xhr, status, error) {
      console.error("[MessengerDM_Live] AJAX Error:", status, error);
