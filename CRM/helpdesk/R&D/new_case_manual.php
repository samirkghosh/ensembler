...
<div class="style2-table">
   ...
   <!-- Farhan Akhtar :: Modified on (16-10-2024) -->
    ...
    <!-- Farhan Akhtar :: Modified on (07-01-2025) :: Design is modified as per Samir Sir -->
   <form method="post" action="" name="customer_form" id="customer_form" autocomplete="autocomplete_off_hack_sfr3!g">
            ...
            <div class="row">
               <div class="col-sm-10">
                  <input type="text" id="search_res" name="search-box" maxlength="100"
                     ...
                     placeholder="CALLER ID" class="select-styl1 search_res"
      ...
      <div class="old-customer-simple-table " <?php if ($module_flag_customer != '1') { ?> style="display: none;" <?php } ?>>
         <div class="breadcrumb_head"> CASE DETAILS </div>
         <table class="tableview tableview-2 main-form new-customer">
            <tbody>
               <tr>
                 <td class="left border0-left">
                    <label>Loanee<em>*</em></label>
                    <input type="text" name="loanee" id="loanee" value="<?= $loanee; ?>" class="input-style1">
                 </td>
                 <td class="left border0-left">
                    <label>Product<em>*</em></label>
                    <input type="text" name="product" id="product" value="<?= $product; ?>" class="input-style1">
                 </td>
                 <td class="left border0-left">
                    <label>Request Type<em>*</em></label>
                    <input type="text" name="request" id="request" value="<?= $request; ?>" class="input-style1">
                 </td>
                 <td class="left border0-left">
                    <label>Department<em>*</em></label>
                    <input type="text" name="department" id="department" value="<?= $department; ?>" class="input-style1">
                 </td>
               </tr>
               <tr>
                <td class="left border0-left">
                    <label>Enquire Type<em>*</em></label>
                    <input type="text" name="enquire" id="enquire" value="<?= $enquire; ?>" class="input-style1">
                 </td>
                  <td class="left boder0-right">
                     <label>Service Category <em>*</em></label>
                     <input type="text" name="type" id="type" value="<?= $type; ?>" class="input-style1">
                  </td>
                  <td class="left  boder0-left">
                     <label>Response Type</label>
                     <input type="text" name="response" id="response" value="<?= $response; ?>" class="input-style1">
                  </td>
                  <td class="left boder0-right">
                     <label>Priority of call</label>
                     <div class="log-case">
               ...
               <td class="left border0-left">
                  <label>Case Title<em>*</em></label>
                  <input type="text" name="case_titile" id="case_titile" value="<?= $case_titile; ?>" class="input-style1">
               </td>
...
/*** Dynamic 365 Web API code start 27-05-2025 [Aarti]***/ 
...
$("#customer_form").on('submit', (function(e) {
  ...
  // ajax call start
    $("#customer_form :input").prop('disabled', false);

    $("#create_ticket").prop('disabled', true);
    $("#create_ticket").val('Please Wait...');

    $.ajax({
       url: 'helpdesk/dynamic_function.php',
       type: "POST",
       ...
       success: function(data) {
          if(data.success){
            $('.successmanual').text('CASE ID :: '+data.case_id);
            $('.successmanual').show();
            $('#docket_no_new').val(data.case_id);
          }else{
...
// Triggered when user types in phone number input
    ...
    if (val.length >= 3) {
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#search_result").css("display", "block").html(data);
            },
...
// Called when a search result row is clicked
function showCustomerDetails(customer) {
    var customer_array = customer.split("||");

    // Fill the form with fetched customer info
    $("#first_name").val(customer_array[0]);
    $("#last_name").val(customer_array[1]);
    $("#phone").val(customer_array[2]);
    $("#email").val(customer_array[3]);
