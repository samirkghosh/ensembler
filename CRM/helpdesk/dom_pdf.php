<?php
/***
 * Download Ticket Details in PDF formats
 * Author: Aarti
 * Date: 09-04-2024
 *  This code is used in a web application to Download Ticket Details in PDF formats
-->
 **/
  // require_once '../dompdf/autoload.inc.php'; 
  require_once('/var/www/html/ensembler/CRM/dompdf/autoload.inc.php');
   $ticket = $ticketid; 
   ...
   $phone=$res['phone'];
   $alternate=$res['mobile'];
   $company_name = $res['company_name'];
   $reg_num = $res['company_registration'];
   $name =  explode(' ', $res['fname']);
   $fname=$name[0];
   $lname=$name[1];
   $agent=agentname($res['i_CreatedBY']);
   $email=$res['email'];
   $province=city($res['district']);
   $district=village($res['v_Village']);
   $address1=$res['address'];
   $address2=$res['v_Location'];
   $gender=get_gender($res['gender']);
   $age=get_age_group($res['age_grp']);
   $complaint=source($res['i_source']);
   $call = $res['call_type'];
   $language = getlanguagename($res['language_id']);
   $facebook=$res['fbhandle'];
   $twitter=$res['twitterhandle'];
   $reason=$res['vCaseType'];
   $priority=$res['priority'];
   $priority_user = ($priority == 0) ? 'Non Priority' : 'Priority';
   $whatsapp_number = $res['whatsapphandle'];
   $sms_number = $res['smshandle'];
   $nationality = $res['nationality'];
   $regional = regional($res['regional']);
   $customer_type = $res['customertype'];
   $category=category($res['vCategory']);
   $subcategory=subcategory($res['vSubCategory']);
   $department=department($res['vProjectID']);
   $status=ticketstatus($res['iCaseStatus']);
...
$html  ='
    ...
    <table style="width: 100%" class="header">
      <tr>
          <td style="width: 32%; vertical-align: left;">
            <img src="../../public/images/alliance_logo.png" style="height:70px;">
          </td>
          <td style="width: 45%; vertical-align: middle;">
          <span style="text-align:left;font-size:20px;color:#003399;font-weight:600">CASE DETAIL - <font color="#993300"> ('.$ticket.')</font></span>
          </td>
          <td style="text-align: right;">
            <h1 style="text-align: right">Date - <font color="#993300">'.$date.'</font></h1>
            <span style="font-weight: bold; font-size: 0.7em;">STATUS:<font color="#993300"> '.$status.' </font></span>
          </td>
      </tr>
    </table>

    <table class="detail" style="margin: 0px; border-top: none;border-bottom: none;">
     <tr style="background: #2e307b;color: #fff;">
       <td colspan="6" style="border: 1px solid #000;font-size: smaller;font-weight: 600;border-left: none;border-right: none;">Customer Profile</td>
     </tr>
        ...
        <tr>
          <td class="label">Priority User:</td>

          <td class="field"> '.$priority_user.'</td>
      ...
      <td class="label">Regional Offices:</td>
      ...
      <td class="label">Facebook Handle:</td>
    ...
    <table class="detail" style="margin: 0px; border-top: none;border-bottom: none;">
      <tr style="background: #2e307b;color: #fff;">
        <td colspan="6" style="border: 1px solid #000;font-size: smaller;font-weight: 600;border-left: none;border-right: none;">Channel Information</td>
      </tr>
      
      <tr>
          <td class="label">Complaint Origin:</td>

          <td class="field">'.$complaint.'</td>
    ...
    <table class="detail" style="margin: 0px; border-top: none;border-bottom: none;">
      <tr style="background: #2e307b;color: #fff;">
        <td colspan="6" style="border: 1px solid #000;font-size: smaller;font-weight: 600;border-left: none;border-right: none;">Other Information</td>
      </tr>
      
        <tr>
          <td class="label">Reasons For Calling:</td>

          <td class="field">'.$reason.'</td>

          <td class="label">Priority of Call:</td>
        ...
        <tr>
          ...
          <td class="label">Assign Department:</td>

          <td class="field">'.$department.'</td>

          
          <td class="label">Status Of Complaint:</td>
        ...
        <tr>
          ...
          <td class="label">Corrective Measure:</td>

          <td class="field">'.$corrective_measure.'</td>
...
';
   ...
   $dompdf->render(); 
    ...
    /*
      ...
      * Purpose :: Save the generated PDF file on Base storage path for multi-tenant
    */

    $directory = '/var/www/html/'.BasePathStorage.'/pdf/';
    if (!is_dir($directory)) {
        mkdir($directory, 0777, true); // Create the directory if it doesn't exist
    }
    file_put_contents($directory.$ticket.'.pdf', $output);

    // $ticketname = str_replace('/', '-', $ticket);
  //  file_put_contents('../pdf/'.$ticket.'.pdf', $output); 
   ?>
