...
// updated the code logi to avoid error [vastvikta][12-03-2025]
function fetchSatisfactionData($query_str_sat) {
    ...
    // Error handling
    if (!$query_satisfaction1) {
        die("Query Failed: " . mysqli_error($link));
    }

    return mysqli_fetch_assoc($query_satisfaction1) ?? ['Total_Calls' => 0, 'GOOD' => 0, 'BAD' => 0];
}
function fetchSatisfactionData2($startdatetime,$enddatetime,$agentquery) {
    ...
    $sql_satifaction2="SELECT count( * ) AS Total_Calls2,
                        ...
                        FROM $db.tbl_civrs_cdr WHERE (Type=1 or Type=2 or Type=3 or Type=4) and Dialed_Digit!='NULL'  $query_str_satq ";
                        // changes the condition for all types i.e. chat and sms [vastvikta][12-03-2025]
...
function fetchNpsData($query_str_nps) {
    global $db, $link; // Assuming $link is your database connection object
...
function fetchCustomerEffortData($query_str_customer) {
    global $db, $link; // Assuming $link is your database connection object
