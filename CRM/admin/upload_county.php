...
<div class="style2-table">
    ...
    <label for="county_excel">Upload Excel File:<em>*</em></label>
    <input type="file" name="county_excel" id="county_excel" accept=".xlsx,xls" required class="input-style1">
    <input type="button" id="upload_button" value="Upload" class="button-orange1">
   
    <a href="admin/sample_document/County.xlsx" download style="margin-left: 10px; text-decoration: none; color: #007bff;">
            ...
            <div class="">
                <div class="div2">
                    <table class="tableview tableview-2" id="admin_table">
                        ...
                        <tbody>
                            <?php
                            $sql = "SELECT * FROM $db.uploaded_file WHERE type = 'county' ";
                            ...
                            <tr>
                                ...
                                <?php if (!empty($res['document_name'])): 
                                    $fileUrl = $SiteURL . "CRM/admin/uploads/county/" . urlencode($res['document_name']);?>
...
function uploadExcel() {
    const fileInput = document.getElementById("county_excel");
    ...
    reader.onload = function (e) {
        try {
            ...
            if (countyIndex === -1) {
                alert("❌ Required column 'County' is missing.");
                return;
            }
            ...
            for (let i = 1; i < rows.length; i++) {
                const row = rows[i];
                const countyName = row[countyIndex]?.toString().trim();

                if (countyName) {
                    counties.push({ county: countyName });
                }
            }

            const formData = new FormData();
            formData.append("action", "upload_county_json");
            formData.append("county", JSON.stringify(counties));
            formData.append("county_excel", file);

            console.log("📦 Data being sent:", counties);
            ...
            .then(result => {
                if (result.success) {
                    alert("✅ All counties uploaded successfully!");
                    location.reload();
                } else {
                    let message = `❌ Counties were not added.\n`;
