<?php
/**
 * Controller for Webchat Template Management
 *
 * @author AI
 * @version 1.0
 * @package ensembler
...
switch ($action) {
    case 'create_webchat_template':
    case 'update_webchat_template':
        $response = handle_webchat_template_submission();
        break;
    case 'delete_webchat_template':
        $response = handle_webchat_template_deletion();
        break;
    case 'get_webchat_templates':
        $response = get_webchat_templates_list();
        break;
    default:
...
function handle_webchat_template_submission() {
    global $link;
    ...
    if ($id) {
        $stmt = $link->prepare("UPDATE tbl_webchat_template SET temp_name = ?, temp_type = ?, temp_content = ? WHERE id = ?");
        $stmt->bind_param("sssi", $template_name, $type, $body, $id);
    } else {
        $stmt = $link->prepare("INSERT INTO tbl_webchat_template (temp_name, temp_type, temp_content) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $template_name, $type, $body);
    }

    if ($stmt->execute()) {
        $message = $id ? 'Webchat template updated successfully.' : 'Webchat template created successfully.';
...
function handle_webchat_template_deletion() {
    global $link;
    $id = isset($_POST['id']) ? filter_var($_POST['id'], FILTER_SANITIZE_NUMBER_INT) : null;
    ...
    $stmt = $link->prepare("DELETE FROM tbl_webchat_template WHERE id = ?");
    $stmt->bind_param("i", $id);

    if ($stmt->execute()) {
        return ['success' => true, 'message' => 'Webchat template deleted successfully.'];
    } else {
        return ['success' => false, 'message' => 'Database error: ' . $stmt->error];
    }
}

function get_webchat_templates_list() {
    $result = view_webchat_template();
    $templates = [];
