<?php
/***
 * Email Module related
 * Author: Aarti Ojha
 * Date: 28-10-2024
 * Description: This file handles check case created with mails
 */
...
if($type == 'email')
{
	...
	$query5 = mysqli_query($link,"select i_reminder,ICASEID,i_DeletedStatus from $dbname.web_email_information where EMAIL_ID='$Mid' ; ");
	$query5 = mysqli_fetch_row($query5);
	$working= $query5[0];  $caseid= $query5[1]; $del = $query5[2];
	if($del==0 && $working==0 && isset($_POST['del'])){ 
		mysqli_query($link,"update $dbname.web_email_information set i_DeletedStatus='2' where EMAIL_ID='$Mid'"); 
		echo ""; 
		exit;
	} 
	...
	{ 
		...
		mysqli_query($link,"update $dbname.web_email_information set i_reminder=0 where EMAIL_ID='$Mid' ; "); echo ""; 
	}
...
{
	if(isset($_POST['del'])){ 
		$query = mysqli_query($link,"update $db.tbl_smsmessagesin set i_status='0' where i_id='$Mid'"); 
	...
	if(isset($_POST['id']))
	{
		$query = mysqli_query($link,"select i_reminder from $db.tbl_smsmessagesin where i_id='$Mid'");
		$query5 = mysqli_fetch_row($query);
		$working= $query5[0];
		if($working==0){ 
			mysqli_query($link,"update $db.tbl_smsmessagesin set i_reminder='1' where i_id='$Mid'"); 
			echo ""; 
			exit;
		}else 
		{ 
