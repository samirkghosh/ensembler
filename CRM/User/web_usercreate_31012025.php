...
// echo"<pre>"; print_r($row);
		...
		// Fetch IP address 
		$sql_ip="select IP_Address from $db.encryp1 where UserName='$atxusername'";
	...
	<div class="style2-table">
		<form method="post" name="adduserfrm" id="adduserfrm" enctype="multipart/form-data">	
			...
			<table cellspacing="0" cellpadding="1" width="98%" border="0" align="left" class="tableview tableview-2 main-form">				
				<tr>
					<td align="left" valign="middle" width="18%" class="boder0-left boder0-right">First&nbsp; Name<em> *</em></td>
					<td width="32%" align="left" valign="middle" class="boder0-left boder0-right">
						<input type="text" name="fname" id="fname" placeholder = "Please enter First Name" class="input-style1 CustomInput" onblur='User.callchange()' value='<?=$fname?>'>
					</td>
					<td align="left" valign="middle" width="17%" class="boder0-right">Last&nbsp; Name <em>*</em></td>
					<td width="33%" align="left" valign="middle" class="boder0-left">
						<input type="text" name="lastname" id="lastname" placeholder = "Please enter Last Name" class="input-style1 CustomInput" onblur='User.callchangelast()' value='<?=$sn?>' onChange="this.value=this.value.trim();">
					</td>
				</tr>
				<tr>
					<td align="left" valign="middle" class="boder0-left boder0-right">Gender<em> *</em></td>
				...
				<?php }else{?>
					<td align="left" valign="middle" class="boder0-right">Password</td>
					<td align="left" valign="middle" class="boder0-left boder0-right">
						<input type="password" name="password" id="password" class="input-style1 CustomInput" value='<?=($_POST[' password ']); ?>' readonly>
					</td>
				<?php }?>
				</tr>
				<tr>
					...
					<td align="left" valign="middle" class="boder0-right">Department<em> *</em></td>
					<td align="left" valign="middle" class="boder0-left boder0-right">
				...
				<tr>
					<td align="left" valign="middle" class="boder0-left boder0-right">Job Title<em> *</em></td>
					<td align="left" valign="middle" class="boder0-left">
					...
					<td rowspan="2" align="left" valign="middle" class="boder0-right">Reports&nbsp;To<em> *</em></td>
					<td rowspan="2" align="left" valign="middle" class="boder0-left boder0-right">
						<select name="boss[]" id="boss" class="select-styl1 CustomInput" multiple="multiple">
						<option value="">Select Reports To</option>
						<?php
							$res=uniuserprofile();
							while($row=mysqli_fetch_array($res)){
								$boss=$row['AtxUserName'];
								$bossid=$row['AtxUserID'];
								$AtxDesignation=$row['AtxDesignation'];
							?>
						<option value="<?=$bossid?>" <? if(in_array($bossid, $reportto))
															echo "selected" ; ?>><?=$boss .' ('.$AtxDesignation.')' ?></option>
						<? }?>
						</select>
					</td>
				</tr>
				<tr>
					<td height="32" align="left" valign="middle" c class="boder0-right">Email<em> *</em></td>
					<td align="left" valign="middle" class="boder0-left">
				...
				<tr>
					<td align="left" valign="middle" class="boder0-left boder0-right">Contact No<em> *</em></td>
					<td align="left" valign="middle" class="boder0-left">
					...
					<td align="left" valign="middle" class="boder0-right">Upload Image</td>
						<td align="left" valign="middle" class="boder0-left boder0-right">
				...
				<tr>
				    <td align="left" valign="middle"  class="boder0-left boder0-right">Skills<em> *</em></td>
				    <td align="left" valign="middle" class="boder0-left ">
				 	 <select name="ddl_skill[]" id="ddl_skill" class="select-styl1 CustomInput" multiple="multiple">
				        <option value="">Select Skills</option><?
						$resskill  = skill_list();
						while($rowskill=mysqli_fetch_array($resskill)){
							$skillid=$rowskill['i_skillID'];
							$skillname=$rowskill['v_SkillName'];
						?>
				        <option value="<?=$skillid?>" <? if(in_array($skillid, $skill)) echo "selected"; ?>><?=$skillname?></option><? }?>
				        </select>
				    </td>
					<td class="boder0-left boder0-right">Status</td>
				...
				<!-- <tr>
					<td class="boder0-right">Extension No</td>
					<td class="boder0-left">
						<input type="text" name="ext" id="ext" class="input-style1 CustomInput" value='<?=$ext?>'>
						</td>
					
				</tr> -->
				<tr>
					<td align="left" valign="middle" class="boder0-left boder0-right">Description</td>
					<td align="left" valign="middle" class="boder0-left">
						<textarea name="desc" id="desc" placeholder = "Enter Description" class="text-area1 CustomInput" value='<?=($_POST[' desc ']);?>' onKeyDown="User.textCounter(document.adduserfrm.desc,document.adduserfrm.remLen1,255)" onKeyUp="User.textCounter(document.adduserfrm.desc,document.adduserfrm.remLen1,255)"><?=$desc?></textarea>
						<br />
					...
					<td align="left" valign="middle" class="boder0-right">Login System IP</td>
					<?
					$localIP = getenv("REMOTE_ADDR");
					?>
					<td align="left" valign="middle" class="boder0-left boder0-right">
						<input type="text" name="ip_address" id="ip_address" class="input-style1 CustomInput" maxlength="15" value="<?=$localIP?>" readonly/>
					</td>
				</tr>
			...
			<?php if($user_group == $Admin_groupId){?> 
            	...
            	<?php } ?>
            	<tr id="channel_div">
					<td align="left" valign="middle" class="boder0-right">Channel Assign<em> *</em></td>
					<td align="left" valign="middle" class="boder0-left boder0-right">
                	...
                	if($module_flag == '1'){?>
					<td align="left" valign="middle"  class=" boder0-right">Shift</td>
				    <td align="left" valign="middle" class="boder0-left boder0-right">
