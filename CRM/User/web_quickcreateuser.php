...
function Create_User(){
	...
	global $wpdb, $db,$link, $domain, $base,$Admin_groupId,$NonLogin_groupId,$Backoffice_groupId,$Link_Login,$from_helpdesk_email,$uc_ip,$licence_Named,$campaign;
	// Fetching necessary data
	...
	// $ip_address = !empty($_POST['ip_address']) ? $_POST['ip_address'] : '0.0.0.0';
	...
	$classify = ($jobtitle == '070000') ? $_POST['classify'] : (($jobtitle == '080000') ? 2 : 1);
	...
	if($jobtitle != $NonLogin_groupId){
		...
		if($jobtitle != $Backoffice_groupId){ // I have commnet code for give access backoffice also
			if(in_array($jobtitle, $title_arr)){
				$response = get_web_page("http://$uc_ip/agc/executequery.php?mode=insert&user=$username_full&pass=$passwd1&emailid=$email&company=$companyid&campaign=$campaign");		
				if($response=='exists'){
					$data['status'] = 'false';
	...
	if(($num_rows["countuser"]>'0')&&($sql3_num>'0')){				
		...
		if($jobtitle ==$NonLogin_groupId){
			...
			// added  user create log  in audit report [vastvikta][13-02-2025]
			add_audit_log($vuserid, 'user_created', '', $username_full.' user created', $db);
...
$vuserid	   	= 	$_SESSION['userid'];
    ...
    $num_rows=mysqli_fetch_array($res_user);
  	if($num_rows["countuser"]>'0'){
		...
		/***check for update***/ 
		$sql_query_check = "SELECT AtxUserStatus,AtxEmail,GivenName,sn,AtxUserName FROM $db.uniuserprofile WHERE AtxUserID='$user_id' " ;	
		...
		$old_fname = $row_check['GivenName'];
		$old_lname = $row_check['sn'];
		$username_old = $row_check['AtxUserName'];
		...
		// uppdated the code so that  date changes when user changes active inactive status [vastvikta][08-01-2025]
		// Check if the previous status was 0 and the current status is 1
		if ($previous_status == 0 && $status == 1) {
			// Update the query to set the JoiningDate to the current date if the condition is met
			$sql1_up = "UPDATE $db.uniuserprofile 
						SET GivenName='$fname', 
							sn='$lastname', 
							AtxUserCat = '$level', 
							AtxUserName='$username_full', 
							AtxDisplayName='$usern', 
							AtxCompany='$company', 
							AtxDesignation='$jobtitle_name', 
							AtxEmail='$email', 
							AtxEmailPwd='$emailpassword', 
							AtxBirthDate='$closedate', 
							AtxGender='$gender', 
							AtxStreet='$hstreet', 
							AtxCity='$hcity', 
							AtxState='$hstate', 
							...
							AtxPinCode='$pincode', 
							AtxHomePhone='$phone', 
							AtxContactPhone='$contactphone', 
							AtxContactFax='$fax', 
							AtxPager='$pager', 
							AtxMobile='$mobile', 
							AtxDescription='$desc', 
							AtxUserStatus='$status', 
							i_Skillset='$ddl_skill1', 
							i_shiftpref='$ddl_shift', 
							i_classify='$classify', 
							LeavingDate='', 
							JoiningDate=CURRENT_DATE 
						WHERE AtxUserID='$user_id'";
						
		} else {
			// Regular update if the status did not change from Inactive to Active
			$sql1_up = "UPDATE $db.uniuserprofile 
						SET GivenName='$fname', 
							sn='$lastname', 
							AtxUserCat = '$level', 
							...
							i_shiftpref='$ddl_shift', 
							i_classify='$classify', 
							LeavingDate='$dateof_leave1' 
						WHERE AtxUserID='$user_id'";
		}
		$res=mysqli_query($link,$sql1_up) or die(" Query Error In uniuserprofile Table ".mysqli_error($link));
		// added  user active inactive log   in audit report [vastvikta][12-03-2025]
		if ($previous_status == 0 && $status == 1) {
			if($status == 1){
				$status_name = 'Active';
			}else{
			...
			add_audit_log($vuserid, 'user_status_update', '', 'Status set to '.$status_name. ' for user '.$username_full, $db);
		}elseif($previous_status == 1 && $status == 0){
			if($status == 1){
				$status_name = 'Active';
			}else{
				$status_name = 'In-Active';
		...
		add_audit_log($vuserid, 'user_update', '', 'User details updated '.$username_full, $db);
		}
// change ip on the basis of name change [vastvikta][12-06-2025]
		if ($username_full != $username_old) {
			$sql_en = "UPDATE $db.encryp1 SET IP_Address='$ip_address', UserName='$username_full' WHERE UserName='$username_old' AND `Status` = 1";
	
			} else {
			
			$sql_en = "UPDATE $db.encryp1 SET IP_Address='$ip_address' WHERE UserName='$username_full' AND `Status` = 1";
			}
...
// name update [vastvikta][12-06-2025]
		$name_update = '';
		if ($old_fname != $fname || $old_lname != $lastname) {
			$name = strtolower($fname . $lastname);

			$name_update = ", user = '$name', full_name = '$name'";
		}

		// [Aarti][18-04-2024] - for change autodial active status 
		$sql_autodial = "UPDATE $db_asterisk.autodial_users 
						SET active_status = '$status' $email_update $name_update 
						WHERE emailid = '$old_email'";
